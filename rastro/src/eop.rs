...
/// Initialized Earth Orientation Data
lazy_static! {
    static ref EOP: RwLock<Option<EarthOrientationData>> = RwLock::new(None);
}

pub fn get_global_eop
...
impl EarthOrientationData {
    ...
    pub fn zero_init() {
        ...
        let mut eop = EOP.write().unwrap();
        *eop = Some(EarthOrientationData {
    ...
    ///
    /// # Examples
    pub fn from_static_values(pm_x: f64, pm_y: f64, ut1_utc: f64, dX: f64, dY: f64, lod: f64) {
        let mut data: HashMap<u32, (f64, f64, f64, Option<f64>, Option<f64>, Option<f64>)> =
            HashMap::new();
    ...
    pub fn from_c04_file(
        ...
        let mut eop = EOP.write().unwrap();
        *eop = Some(eop_c04_from_bufreader(reader, extrapolate, interpolate)?);
    ...
    /// println!("{}", eop);
    /// ```
    pub fn from_default_c04(extrapolate: EOPExtrapolation, interpolate: bool) {
        let reader = BufReader::new(PACKAGED_C04_FILE);

        let mut eop = EOP.write().expect("EarthOrientation RwLock is poisoned.");
        *eop = Some(
            eop_c04_from_bufreader(reader, extrapolate, interpolate)
                .expect("Failed to initialize Earth Orientation Data"),
        );
    }
    ...
    pub fn from_standard_file(
        ...
        let mut eop = EOP.write().expect("EarthOrientation RwLock is poisoned.");
        *eop = Some(eop_standard_eop_from_bufreader(
            reader,
            extrapolate,
            interpolate,
            eop_type,
...
pub fn get_ut1_utc(mjd: f64) -> Result<f64, String> {
    ...
    let eop = EOP
        .read()
        .expect("EarthOrientation RwLock is poisoned.")
        .unwrap();
    // let eop = match EOP {
    //     Some(s) => s.read().expect("EarthOrientation RwLock is poisoned."),
    //     None => {
    //         Err("Unable to retrieve EOP values. No Earth orientation data has been initialized.")
    //     }
    // };
...
/// ```
pub fn get_pm(mjd: f64) -> Result<(f64, f64), String> {
    // Acquire read lock for EarthOrientation Data
    let eop = EOP
        .read()
        .expect("EarthOrientation RwLock is poisoned.")
    ...
    if mjd < (*eop).mjd_max as f64 {
        ...
        match (*eop).extrapolate {
            ...
            EOPExtrapolation::Hold => {
                ...
                Ok((
                    (*eop).data[&(*eop).mjd_max].0,
                    (*eop).data[&(*eop).mjd_max].1,
                ))
            }
            EOPExtrapolation::Error => Err(format!(
...
/// ```
pub fn get_dxdy(mjd: f64) -> Result<(f64, f64), String> {
    let eop = EOP
        .read()
        .expect("EarthOrientation RwLock is poisoned.")
        .unwrap();
...
/// ```
pub fn get_lod(mjd: f64) -> Result<f64, String> {
    let eop = EOP
        .read()
        .expect("EarthOrientation RwLock is poisoned.")
        .unwrap();
...
/// ```
#[allow(non_snake_case)]
pub fn get_eop(mjd: f64) -> Result<(f64, f64, f64, f64, f64, f64), String> {
    let (pm_x, pm_y) = get_pm(mjd)?;
    let ut1_utc = get_ut1_utc(mjd)?;
    let (dX, dY) = get_dxdy(mjd)?;
    let lod = get_lod(mjd)?;
...
impl Default for EarthOrientationData {
    ...
    fn default() -> Self {
        ...
        eop_standard_eop_from_bufreader(
            reader,
            EOPExtrapolation::Zero,
            true,
            EOPType::StandardBulletinA,
        )
        .expect("Failed to parse and load packed Standard Earth Orientation Data.")
    }
}
...
mod tests {
    ...
    fn test_from_c04_file() {
        ...
        // Assert default EOP loaded
        let eop = EOP.read().expect("").unwrap();
        ...
        // EarthOrientationData::from_c04_file(
    ...
    //     let eop_return = EarthOrientationData::from_standard_file(
    //         filepath.to_str().unwrap(),
    //         EOPExtrapolation::Hold,
    //         true,
    //         EOPType::StandardBulletinA,
    ...
    //     let eop = EarthOrientationData::from_default_standard(
    //         EOPExtrapolation::Hold,
    //         true,
    //         EOPType::StandardBulletinA,
    //     );
    ...
    //         .join("test_assets")
    //         .join("iau2000A_finals_ab.txt");
    //     let eop = EarthOrientationData::from_standard_file(
    //         filepath.to_str().unwrap(),
    //         eop_extrapolation,
    //         eop_interpolation,
    //         eop_type,
